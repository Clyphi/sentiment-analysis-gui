...
# --- MQF-Berechnung definieren ---
def mqf_berechnen(model, X, y):
    # Genauigkeit (Accuracy)
    accuracy = model.score(X, y)
    
    # F1-Score (Durchschnitt für alle Klassen)
    y_pred = model.predict(X)
    f1 = f1_score(y, y_pred, average='weighted')
    
    # Robustheit (5-fache Cross-Validation)
    cv_scores = cross_val_score(model, X, y, cv=5)
    robustness = cv_scores.mean()
    
    # Interpretierbarkeit (Feature Importance)
    if hasattr(model, 'feature_importances_'):
        top_features = sum(1 for imp in model.feature_importances_ if imp > 0.01)
        interpretability = top_features / len(model.feature_importances_)
    else:
        interpretability = 0.5  # Falls nicht verfügbar (z. B. bei SVM)
    
    # MQF als gewichteter Durchschnitt
...
def sentiment_train(filepath):
    ...
    accuracy = sentiment_model.score(X_test, y_test)
     # MQF berechnen
    mqf_results = mqf_berechnen(sentiment_model, X_train, y_train)
    
    # Ergebnis als String formatieren
    mqf_str = "\nMQF (Modular Quality Factor):\n"
    mqf_str += f"- MQF Score: {mqf_results['MQF']:.2f}\n"
    mqf_str += f"- Genauigkeit: {mqf_results['Accuracy']:.2f}\n"
    mqf_str += f"- F1-Score: {mqf_results['F1-Score']:.2f}\n"
    mqf_str += f"- Robustheit (CV): {mqf_results['Robustheit (CV)']:.2f}\n"
    mqf_str += f"- Interpretierbarkeit: {mqf_results['Interpretierbarkeit']:.2f}\n"
...
# --- Wichtige Features anzeigen (Interpretierbarkeit) ---
def zeige_wichtige_features(model, vokabular, n=10):
    if hasattr(model, 'feature_importances_'):
        features = sorted(zip(vokabular, model.feature_importances_), key=lambda x: -x[1])
        print("\nTop Features für Sentiment-Entscheidung:")
        for wort, importance in features[:n]:
            print(f"{wort}: {importance:.4f}")
    else:
        print("Modell unterstützt keine Feature Importance (z. B. SVM).")
